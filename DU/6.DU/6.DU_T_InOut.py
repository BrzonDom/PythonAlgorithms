...
def Str_Spl_IntArr(str):

    res = [int(element) for element in str.split()]

    return res

def IntMat_BinMat(mat_int):
...
for line in mat_str:
    ...
    mat_int.append(Str_Spl_IntArr(line))
...
# out_file.write: mat_bin

mat_bin = IntMat_BinMat(mat_int)
print("\nMat_bin print:")
...
# vis_file.write: mat_bin
print(f"\nMat_bin visual {len(mat_bin[0])}:")

for a, arr in enumerate(mat_bin):
